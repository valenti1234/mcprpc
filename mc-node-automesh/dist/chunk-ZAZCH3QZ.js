...
var AutoMesh = class _AutoMesh {
  ...
  constructor(options) {
    ...
    this.meshId = this.options.meshId ?? envMeshId ?? crypto.randomUUID().replaceAll("-", "");
  ...
  static normalizeEndpoint(endpoint) {
    ...
    if (script.startsWith("./") || script.startsWith("../") || !path2.isAbsolute(script) && script.includes("/")) {
      const resolved = path2.resolve(script);
